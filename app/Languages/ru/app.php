<?php

/*
* The following language strings are used to translate the site core 
* (controllers and templates).
*
* Следующие языковые строки используются для переводя ядра сайта 
* (контроллеры и шаблоны).
*/

return [
	'home'              => 'Главная',
	'email'           	=> 'Email',
	'password'			=> 'Пароль',
	'remember_me'		=> 'Запомнить меня',
	'forgot_password'	=> 'Забыли пароль',
	'authorization'		=> 'Авторизация',
	'tools' 			=> 'Инструменты',
	'admin' 			=> 'Панель',
	'auth_yes'			=> 'Вы авторизировались, как персонал. <br>	Это закрытый раздел.',
	'structure'			=> 'Структура',
	'fact'				=> 'Факт',
	'facts'				=> 'Факты',
	'latest_facts'		=> 'Последние факты',
	'add_fact'			=> 'Добавить факт',
	'edit_fact'			=> 'Изменить факт',
	'soft'              => 'Софт',
	'viewing' 			=> 'Просмотр',
	'read_more' 		=> 'Читать далее',
	'not_published' 	=> 'Не опубликован',
	'children' 			=> 'Дети',
	'author' 			=> 'Автор',
	'recommended' 		=> 'Рекомендовано',
	
	'sign_in' 			=> 'Войти',
	'password_recovery' => 'Восстановить пароль',
	'reset' 			=> 'Сбросить',

	'add'				=> 'Добавить',
	'edit'           	=> 'Изменить',
	'characters'		=> 'символов',
	'category'			=> 'Категория',
	'add_category'		=> 'Добавить категорию',
	'edit_category'		=> 'Изменить категорию',
	'title'				=> 'Название',
	'posted'			=> 'Публикация',
	'bold'				=> 'Жирный',
	'italic'			=> 'Курсив',
	'strikethrough'		=> 'Зачеркивание',
	'H2'				=> 'H2',
	'H3'				=> 'H3',
	'checklist'			=> 'Список',
	'quote' 			=> 'Цитата',
	'table'				=> 'Таблица',
	'inline_code'		=> 'Код',
	'code_block'		=> 'Блок кода',
	'view' 				=> 'Просмотр',
	'link'				=> 'Ссылка',
	'foto'				=> 'Фото',
	'switching'			=> 'Переключение',
	'editing'			=> 'Ищменить',
	'fullscreen'		=> 'Полный скрин',
	'note'				=> 'Заметка',
	'source'			=> 'Источник',
	'source_title'		=> 'TITLE источника',
	'source_url'		=> 'URl источника',

	'rebuild_resources'	=> 'Перестроить ресурсы',
	'rebuild'           => 'Перестроить',
	'page_rebuilt'		=> 'Перестроена',
	'transfer photo'	=> 'Перенести Фото',
	'rebuild_dir' 		=> 'DIR',
	'rebuild_title'		=> 'Пути (path) в категориях',
	'rebuild_css_title'	=> 'Стили (JS / CSS)',
	'search_index'		=> 'Индекс',
	'rebuild_html_dir'	=> 'Индексные страницы DIR',
	'rebuild_view' 		=> 'Страницы фактов',
	'deletion_dir' 		=> 'Удаление DIR',
	'delete' 			=> 'Удалить',
	'feed_facts' 		=> 'Лента фактов',
	'no_thumb' 			=> 'Фотографии для факта нет',

	'no_content' 		=> 'Содержимого нет...',

	/* Дата и время + события */
	'months'        => ['', 'Января', 'Февраля', 'Марта', 'Апреля', 'Мая', 'Июня', 'Июля', 'Августа', 'Сентября', 'Октября', 'Ноября', 'Декабря'],
	'today'         => 'Сегодня',
	'yesterday'     => 'Вчера',
	
	'meta_description'	=> 'Описание для поисковых систем...',
	'information' 		=> 'Короткое описание...',
];
